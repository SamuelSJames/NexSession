...
session="$1"
if [ -z "$session" ];then
    session=$(nex_control get_session_path)
    if [ -z "$session" ];then
...
nex_control has_option session_scripts && reput_session_scripts=true
nex_control has_option bookmark_session_folder && reput_bookmarks=true
nex_control set_options not_session_scripts not_bookmark_session_folder

# close and re-open the session
# It prevents to have to save the session with clients off
nex_control close
nex_control open_session_off "$session"
nex_control take_snapshot "Just before NSM conversion"

cd "$(nex_control get_session_path)"
...
group_replaces=''
nexjackpatch_client_id=''

# list clients and operate on them
for client_id in $(nex_control list_clients);do
    # change the client prefix to change their working directory
    # and adapt it to the nsmd way: client_name.client_id
    nex_control client $client_id change_prefix client_name
    
    # read all needed client properties
    properties=$(nex_control client "$client_id" get_properties)
    ...
    case $protocol in
        NSM)
            if [[ "$executable" == nex-jackpatch ]];then
                connections_file="$client_name.$client_id.xml"
                nexjackpatch_client_id="$client_id"
                continue
            fi
            nsm_file_contents+="$client_name:$executable:$client_id\n"
            ;;
        Nex-Hack)
            # we will adapt the Nex-Hack client with nsm-proxy
            arguments=$(get_property arguments|sed 's/$NEX_JACK_CLIENT_NAME/$NSM_CLIENT_ID/g')
            config_file=$(get_property config_file|sed "s/\$NEX_SESSION_NAME/${session_path##*/}/g")
...
"
            ...
            if [ -d "$linkdir" ] && [ -L "$linkdir" ] && [[ "$(readlink "$linkdir")" == "$client_name.$client_id" ]];then
                ...
                # link the NSM Proxy new directory to the Nex-Hack client dir
    ...
    if [[ "$jack_naming" != 1 ]];then
        # the way to name JACK clients is different in NexSession and NSM
        ...
        nex_control client $client_id set_properties jack_naming:1
...
jackpatch_id=nWASNEX

if $replace_jackpatch and [ -n "$nexjackpatch_client_id" ];then
    # remove nex-jackpatch from session and replace it with jackpatch
    nex_control client $nexjackpatch_client_id trash
    nex_control add_executable jackpatch not_start client_id:$jackpatch_id prefix_mode:client_name
fi

if [ -n "$connections_file" ];then
    # get the connections as written by the NSM jackpatch
    # Note it will also update the nex-jackpatch file with new connections
...
$reput_session_scripts && nex_control set_options session_scripts
$reput_bookmarks && nex_control set_options bookmark_session_folder
