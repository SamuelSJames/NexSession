...
jack_mixer_rh_to_nsm(){
    ...
    # trash and remove jack_mixer Nex-Hack client
    nex_control client $client_id trash
    nex_control trashed_client $client_id remove_keep_files

    # add the new jack_mixer NSM client
    nex_control add_executable $executable client_id:$client_id not_start
}
...
config_file="\$NEX_SESSION_NAME.xml"
arguments="-c \"\$CONFIG_FILE\""

list_filters[0]="executable:$executable"
list_filters[1]="protocol:Nex-Hack"
...
jack_mixer_nsm_templates=$(nex_control list_factory_client_templates executable:$executable protocol:NSM)
...
if $convert_current_session;then
    ...
    clients=$(nex_control list_clients "${list_filters[@]}")
...
if $convert_sessions;then
    ...
    for session in $(nex_control list_sessions);do
        ...
        nex_control open_session_off "$session" 2>/dev/null
        
        clients=$(nex_control list_clients "${list_filters[@]}")
        
        if [ -z "$clients" ];then
            nex_control abort 2>/dev/null
            continue
        fi
...
if $convert_client_templates;then
    ...
    IFS=$'\n'
    for template in $(nex_control list_user_client_templates "${list_filters[@]}");do
        unset IFS
        session=$(mktemp -u)
        nex_control open_session_off "$session"
        cd "$session"

        client_id=$(nex_control add_user_client_template "$template" not_start)
        jack_mixer_rh_to_nsm
        nex_control client $client_id save_as_template "$template"
        nex_control abort
    done
fi
...
if $convert_session_templates;then
    ...
    IFS=$'\n'
    for session_template in $(nex_control list_session_templates);do
        session=$(mktemp -u)
        nex_control open_session_off "$session"
        cd "$session"
        echo cd $PWD
        ...
        nex_control save
        nex_control save_as_template "$session_template"
        nex_control abort
    done
fi
