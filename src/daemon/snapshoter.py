...
class Snapshoter(QObject):
    ...
    def _write_exclude_file(self) -> nex.Err:
        if self.session.path is None:
            return nex.Err.NO_SESSION_OPEN
        ...
        contents = (
            "# This file is generated by nex-daemon at each snapshot\n"
        ...
        session_ignored_extensions = nex.GIT_IGNORED_EXTENSIONS
    ...
    def _save_step_1(self):
        ...
        if self._n_file_changed:
            if not self._run_git_process('commit', '-m', 'nex'):
                return
        ...
        if (self._n_file_changed
            ...
            if not self._run_git_process('tag', '-a', ref, '-m', 'nex'):
                return
            ...
            if client_id and session_name != self.session.name:
                ...
                if (client
                        and (client.prefix_mode
                             is nex.PrefixMode.SESSION_NAME)):
                    continue
    ...
    def load_client_exclusive(self, client_id: str, snapshot: str,
        ...
        root = self._get_history_xml_root()
        if root is None:
            self._error_function(nex.Err.NO_SUCH_FILE,
    ...
    def set_auto_snapshot(self, bool_snapshot: bool):
        ...
        if bool_snapshot:
            ...
            if not file_exists:
                contents = "# This file prevent auto snapshots for this session (NexSession)\n"
