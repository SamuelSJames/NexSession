...
class Protocol(Enum):
    ...
    def to_string(self) -> str:
        if self is self.NEX_HACK:
            return 'Nex-Hack'
        if self is self.NEX_NET:
            return 'Nex-Net'
    ...
    def from_string(string: str) -> 'Protocol':
        lo_str = string.lower()
        if lo_str in ('nex_hack', 'nex-hack'):
            return Protocol.NEX_HACK
        if lo_str in ('nex_net', 'nex-net'):
            return Protocol.NEX_NET
        if lo_str == 'internal':
...
def add_self_bin_to_path():
    # Add NexSession/src/bin to $PATH to can use nex executables after make
    # Warning, will works only if link to this file is in NexSession/*/*/*.py
...
class ClientData:
    ...
    in_terminal = False
    nex_hack: 'Optional[NexHack]' = None
    nex_net: 'Optional[NexNet]' = None
    ...
    def set_nex_hack(self, nex_hack: 'NexHack'):
        self.nex_hack = nex_hack

    def set_nex_net(self, nex_net: 'NexNet'):
        self.nex_net = nex_net
    ...
    def prettier_name(self) -> str:
        if self.label:
            return self.label
        if (self.protocol is not Protocol.NEX_HACK
                and self.name):
            return self.name
        return self.executable_path
    
    @property
    def is_nex_hack(self) -> bool:
        return self.protocol is Protocol.NEX_HACK
    
    @property
    def is_nex_net(self) -> bool:
        return self.protocol is Protocol.NEX_NET
