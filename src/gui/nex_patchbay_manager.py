...
class NexPatchbayCallbacker(Callbacker):
    def __init__(self, manager: 'NexPatchbayManager'):
    ...
    def client_show_gui(self, group_id: int, visible: int):
        ...
        for client in self.mng.session.client_list:
            if group_belongs_to_client(group.name, client.jack_client_name):
                ...
                self.mng.send_to_daemon(
                    f'/nex/client/{show}_optional_gui',
                    client.client_id)
                break
...
class NexPatchbayManager(PatchbayManager):
    main_win: 'MainWindow'
    ...
    def _setup_canvas(self):
        ...
        self.app_init(self.main_win.ui.graphicsView,
                      tuple(theme_paths),
                      callbacker=NexPatchbayCallbacker(self),
