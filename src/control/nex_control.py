...
def get_daemon_list() -> list[Daemon]:
    try:
        with open('/tmp/NexSession/multi-daemon.json') as f:
...
if True:
    ...
    dport = os.getenv('NEX_CONTROL_PORT')
    ...
    if (daemon_started
        ...
        session_root = "%s/NexSessions" % os.getenv('HOME')
        try:
            settings_file = open(
                "%s/.config/NexSession/NexSession.conf" % os.getenv('HOME'), 'r')
        ...
        process_args = ['nex-daemon', '--control-url', str(server.url),
