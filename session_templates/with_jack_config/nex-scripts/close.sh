...
if [[ "$NEX_FUTURE_SCRIPTS_DIR" != "$NEX_SCRIPTS_DIR" ]] &&\
        ! [ -f "$NEX_FUTURE_SCRIPTS_DIR/.jack_config_script" ];then
    close_all_if_needed=close_all
fi

nex_control run_step $close_all_if_needed

if [ -n "$close_all_if_needed" ];then
    nex-jack_config_script putback && nex_control hide_script_info
fi
