...
image::../../en/images/nexsession.svg["icon", 100, 100]

_pour la version 0.14.1_

== Introduction
NexSession est un gestionnaire de session audio pour GNU/Linux. Il permet de démarrer dans une même session plusieurs programmes audio, de sauvegarder leurs projets ensemble et ainsi d’éviter de multiples manipulations pour retourner à une configuration donnée.

Pour être lancés dans NexSession, il est très largement préférable que ces programmes audio soient compatibles avec le protocole NSM, et beaucoup le sont déjà, entre autres Ardour, Qtractor, Carla, Guitarix, Mamba, Patroneo, ZynAddSubFx...

NexSession part du principe que votre configuration audio est fonctionnelle pour de la production audio, si ce n’est pas le cas réglez d’abord ce problème et ne perdez pas votre temps à essayer d’utiliser NexSession.
...
Pour la production audio sous GNU/linux, Il est largement conseillé d’utiliser le serveur JACK. Avant de créer ou de lancer une session, assurez-vous que le serveur JACK tourne, NexSession n’a aucun rapport direct avec JACK, toutefois les programmes que vous allez lancer dans votre session en auront besoin.
...
En haut à droite vous apercevez le dossier racine des sessions, c’est là que les sessions seront sauvegardées (une session NexSession étant un dossier contenant un fichier `nexsession.xml`).
...
Servez-vous maintenant de la <<patchbay>> de JACK qui occupe la partie droite de la fenêtre de NexSession.
...
* selon le type et les capacités du client vous pouvez voir ici
    ...
    ** un bouton *Hack*, celà signifie que le client n'est pas compatible NSM, en tout cas qu'il n'est pas lancé avec ce protocole. Cliquer sur Hack permet de changer la façon dont il est lancé en ouvrant la fenêtre de propriétés du client sur l'onglet Nex-Hack
...
* l'état du client qui peut être
    ...
    ** *lancé* : s'il reste sur l'état lancé, celà signifie
        *** si c'est un client Nex-Hack, qu'il n'a pas de fichier de configuration
...
* la case à cocher *Usine* affiche les modèles intégrés à NexSession ou fournis par votre distribution
...
* *Nex-Hack* affiche les clients lancés sans protocole NSM
...
Par exemple, depuis la fenêtre principale de NexSession, pour ajouter Carla tapez _Ctrl+A_, puis `carla`, sélectionnez le bon modèle avec les flèches Haut/Bas, puis Entrée.
...
...
Décocher la case *Protocole NSM* revient à lancer le programme avec le pseudo-protocole Nex-Hack.
...
NexSession n'inclut pas d'outils pour configurer le serveur JACK, *QJackCtl*, *Studio Control*, *Cadence*, ou l'utilitaire en ligne de commande `jack_control` font très bien ce travail. Notez que cette baie de brassage fonctionne également avec PipeWire.
...
Selon le protocole du client, le deuxième onglet s'appelle NSM, Nex-Hack ou Nex-Net.
...
Si vous tenez à connaître le fichier .desktop utilisé, tapez `nex_control client IDENTIFIANT get_properties` dans un terminal (en remplaçant IDENTIFIANT par l'identifiant du client).
...
* la case à cocher *Empêcher l'arrêt sans sauvegarde récente ou possible* concerne la fenêtre qui peut apparaître lorsque vous demandez à un client de s'arrêter.
    ...
    Si la case est cochée, la fenêtre vous previendra lorsque
        ...
        ** le client est non-sauvegardable depuis NexSession
...
Cochez la case *Lancer dans un terminal* si vous souhaitez obtenir des informations de débogage sur le programme. Celà peut faire légèrement varier le comportement du client, notamment la façon dont il est arrêté. Pour choisir l'application de terminal qui exécute les programmes lancés dans un terminal, rendez-vous dans la fenêtre de préférences de NexSession.



=== Onglet *Nex-Hack*

Si le client est de type Nex-Hack, ici moult champs s'offrent à vous. Ce n'est pas forcément une bonne nouvelle, l'idée est de pouvoir charger dans la session un programme qui n'est pas (encore) compatible NSM. S'il est bien implémenté dans le client, le protocole NSM sera toujours bien plus confortable à utiliser et plus fiable que cette bidouille. Ceçi dit, si l'on peut attendre l'implémentation NSM dans tous les programmes audio, ce n'est pas le cas des autres programmes qui peuvent tout de même avoir une utilité dans la session.

Le pseudo-protocole Nex-Hack reprend les attributs des proxy (nsm-proxy ou nex-proxy), sauf que le client est lancé directement dans la session.

image::proprietes_nex_hack.png[Propriétés Nex-Hack d'un client]

==== Bloc de lancement

image::nex_hack_lancement.png[]
...
La variable `$NEX_SESSION_NAME` sera automatiquement remplacée par le nom de la session. +
...
Avec `nex_control` vous pouvez affecter d'autres signaux que ceux proposés dans les boîtes à choix multiple.
Par exemple +
`nex_control client CLIENT_ID set_properties save_sig:22` +
...
Vous devrez lancer un démon sur la machine distante avec la commande `nex-daemon -p 1234` (`1234` est un exemple, mettez le port que vous voulez). Ce démon vous affiche dans le terminal quelque chose sous cette forme:
```
[nex-daemon]URL : osc.udp://192.168.1.00:1234/
[nex-daemon]      osc.udp://nom-de-machine:1234/
[nex-daemon]ROOT: /home/utilisateur/NexSessions reseau
```
...
Vous avez à présent 2 fenêtres NexSession sur votre machine maître, l'une contrôle la session maître, l'autre l'esclave. Vous reconnaîtrez l'esclave au fait qu'elle n'a ni barre d'outils (*Nouvelle Session*, *Ouvrir une session*, *Contrôle*), ni boutons *Abandonner la session* et *Fermer la session*.
...
Si vous lancez `nexsession -p 1234` sur votre machine esclave, vous aurez la fenêtre de la session esclave en double exemplaire, une sur chaque machine.

*Conseil:* Mettez ce `nex-daemon -p 1234` au démarrage de votre machine esclave.
...
Un nouveau client est créé, sa fenêtre de propriétés s'ouvre sur l'onglet Nex-Hack et Audacity se lance.

Dans Audacity, nous allons directement sauvegarder un projet vide dans le dossier du client. Le dossier du client se trouve dans le dossier de la session et porte le nom donné après *Dossier:* tout en haut de l'onglet Nex-Hack. Nous allons appeler le projet EXACTEMENT comme la session NexSession en cours.
...
Fermez Audacity. +
En haut à droite de l'onglet Nex-Hack de la fenêtre de propriétés du client, cliquez sur *Naviguer*. +
...
Si tout s'est bien passé, le champ *Fichier de configuration* est devenu `$NEX_SESSION_NAME.aup` et le champ *Arguments* est devenu `"$CONFIG_FILE"`. +
...
image::audacity_nex_hack_final.png[Onglet Nex-Hack du client audacity dûment rempli]
...
Notez que le bouton de sauvegarde du client est derrière un point d'exclamation orange, celà signifie que NexSession n'est pas en mesure de sauvegarder son projet et qu'il faudra le faire vous même.
...
Pour importer une session créée avec *Non Session Manager* ou *New Session Manager*, déplacez ou copiez le dossier de la session dans le dossier racine des sessions NexSession (par défaut ~/NexSessions). Ensuite cliquez sur *Ouvrir une session*, votre session doit apparaître dans la liste des sessions, double-cliquez dessus.

NexSession ne ré-écrira pas les clients ajoutés ou supprimés dans la fichier session.nsm, à partir du moment où vous ouvrez une session NSM avec NexSession, vous devez continuer avec NexSession.
...
Si cette option est enclenchée, NexSession sauvegardera (ou tentera de sauvegarder) le numéro du bureau virtuel sur lequel se trouvaient les fenêtres des clients à la sauvegarde de la session. +
...
Les scripts de session se trouvent dans un dossier `nex-scripts` situé soit dans un dossier de la session, soit dans un dossier parent. +
Par exemple, pour une session étant dans: +
`~/NexSessions/avec_script_de_foo/ma session`

le dossier des scripts de session peut-être +
`~/NexSessions/avec_script_de_foo/ma session/nex-scripts` +
`~/NexSessions/avec_script_de_foo/nex-scripts` +
`~/NexSessions/nex-scripts` +
`~/nex-scripts`
...
Seul le dossier de scripts le plus près de la session dans l'arborescence sera considéré. Ainsi, un dossier `nex-scripts` vide dans une session désactivera les scripts pour cette session.

Pour éditer les scripts, commencez par créer une session avec le modèle *avec les scripts de base*, il s'agit d'un modèle de session avec des scripts ne comprenant absolument aucune action particulière. Rendez-vous dans le dossier `nex-scripts` dans le dossier de la session, vous y trouverez les fichiers `load.sh`, `save.sh` et `close.sh`. Dans chacun de ces scripts, `nex_control run_step` correspond à l'action normale effectuée (selon le script: charger, sauvegarder ou fermer la session). Si un de ces trois scripts ne vous est d'aucune utilité, supprimez le, ce sera du temps gagné de ne pas passer par ce script.

Les fichiers scripts doivent impérativement être exécutables pour fonctionner.

Vous aurez probablement besoin de l'utilitaire en ligne de commande `nex_control` pour effectuer des actions en rapport avec tel ou tel client. tapez `nex_control --help` pour connaître toutes ses possibilités.
...
Il s'agit en fait d'une session scriptée (voir <<les_scripts_de_session>>) qui lance un script fourni avec NexSession, mais qui lui est complètement externe, ainsi NexSession n'a toujours aucun rapport direct avec JACK.
...
* copiez le dossier `nex-scripts` d'une session avec mémoire de la configuration de JACK dans le dossier de la session
* Activez l'option <<option_scripts_de_session>>
* Ouvrez la session

*ou*

* déplacez la session dans un sous-dossier contenant le dossier `nex-scripts` adhoc
* Ouvrez la session
...
NexSession n'est en réalité qu'une interface graphique pour nex-daemon. Lorsque vous lancez NexSession, l'interface graphique lance le démon et s'y connecte, et elle arrête le démon lorsqu'on la ferme. L'interface graphique et le démon communiquent entre eux par messages OSC (Open Sound Control), comme c'est le cas entre le démon et les clients NSM. Ainsi, vous pouvez connecter plusieurs interfaces graphiques à un démon, même à distance. Tapez `nexsession --help` pour voir comment faire.

Il n'est pas interdit d'avoir plusieurs instances de démon lancés simultanément, ainsi si vous lancez NexSession alors qu'une instance est déjà lancée, celle-ci lancera un nouveau démon. Toutefois, cette façon de travailler étant peu commune, l'utilisation d'un seul démon est favorisée. Ainsi, si un démon tourne et qu'il n'a aucune interface graphique attachée, nexsession se connectera à ce démon par défaut.

[#nex_control]
== Contrôler NexSession en ligne de commande

la commande `nex_control` vous permet de faire a peu près tout ce qu'il est possible de faire avec l'interface graphique, et même un peu plus. tapez `nex_control --help-all` pour connaître l'ensemble des possibilités.

Au cas où il y a plusieurs démons lancés (voir <<sous_le_capot>>), `nex_control` ne considerera que celui qui a été lancé en premier, sauf si vous spécifiez son port OSC avec l'option `--port` ou la variable d'environnement NEX_CONTROL_PORT.

On pourrait penser qu'il n'y a pas d'intérêt à utiliser `nex_control` puisque la commande `oscsend` permet d'envoyer un message OSC au démon, c'est faux. +
D'une part, parce que `oscsend` permet d'envoyer des messages mais pas d'obtenir des informations de manière simple (quels sont les clients actifs ? quel est l'executable de tel client ? ...), d'autre part parce que la commande `nex_control` se terminera lorsque l'action demandée sera effectuée, par exemple `nex_control open_session ma_session` se terminera lorsque la session sera chargée.

Pensez à affecter `nex_control save` à un raccourci global de votre environnement de bureau (_Ctrl+Meta+S_), celà vous fera gagner un temps fou !

== Foire aux questions

- *Est-il toujours intéressant de lancer Ardour (ou un autre DAW compatible NSM) directement plutôt que dans NexSession ?*

+
Sauf pour un projet vraiment tout petit, Non. +
Si vous utilisez Ardour, lancez le toujours depuis NexSession, d'une part le cliché automatique à la sauvegarde peut vous être d'un secours inespéré, d'autre part, vous n'êtes pas à l'abri d'avoir besoin d'un autre programme même si vous ne l'aviez pas prévu.

- *Puis-je lancer dans une session NexSession un projet Ardour déjà existant ?*

+
Vous trouverez dans le menu *Fichier -> Utilitaires -> Convertir une session Ardour en session Nex*, celà crée et lance une session NexSession à partir d'une session Ardour. Il n'est pas surfait de faire une copie de sauvegarde de votre session Ardour avant de lancer ce script. Bien évidemment, il ne faut pas que Ardour soit ouvert avec cette session pendant l'execution de ce script, mais ça vous l'aviez déjà compris.
